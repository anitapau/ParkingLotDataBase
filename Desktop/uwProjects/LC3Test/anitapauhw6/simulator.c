...
int main(int argc, const char * argv[]) {
	...
	*/
	...
	//BitString value;
	//BSTR_SetBits(&value, "0000000000000011");
	//COMP_LoadWord(&comp, 7, value);
	//BitString notInstr;
	//BSTR_SetBits(&notInstr, "0010010000000111");
	//COMP_LoadWord(&comp, 0, notInstr);
	//BitString trapInstr;
	//BSTR_SetBits(&trapInstr, "1111000000100001");
	//COMP_LoadWord(&comp, 0, trapInstr);
	
	//COMP_Display(comp);


	//BitString notInstr;
	//BSTR_SetBits(&notInstr, "0010000000000111");
	//COMP_LoadWord(&comp, 0, notInstr);
	//COMP_Execute(&comp);

	//BitString trapInstr;
	//BSTR_SetBits(&trapInstr, "1111000000100001");
	//COMP_LoadWord(&comp, 1, trapInstr);
	int j;
	j = 0;
	BitString instr;
	for (j = 0; j < programSize; j++) {
		
		BSTR_SetBits(&instr, program[j]);
		COMP_LoadWord(&comp, j, instr);
	}
	COMP_Execute(&comp);
	COMP_Display(comp);
	/* execute the program */
